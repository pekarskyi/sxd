...
// Додаємо заміну для застарілої функції у PHP 8+
...
// Додаємо адаптер для mysql_* функцій, якщо вони не існують
if (!function_exists('mysql_connect')) {
    // Глобальне з'єднання для емуляції поведінки mysql_*
    $GLOBALS['__MYSQLI_CONNECTION'] = null;
    
    function mysql_connect($server, $username, $password) {
        $server_parts = explode(':', $server);
        $host = $server_parts[0];
        $port = isset($server_parts[1]) ? $server_parts[1] : 3306;
        
        $GLOBALS['__MYSQLI_CONNECTION'] = @mysqli_connect($host, $username, $password, '', $port);
        return $GLOBALS['__MYSQLI_CONNECTION'] ? true : false;
    }
    
    function mysql_select_db($database) {
        return mysqli_select_db($GLOBALS['__MYSQLI_CONNECTION'], $database);
    }
    
    function mysql_query($query) {
        return mysqli_query($GLOBALS['__MYSQLI_CONNECTION'], $query);
    }
    ...
    function mysql_affected_rows() {
        return mysqli_affected_rows($GLOBALS['__MYSQLI_CONNECTION']);
    }
    
    function mysql_error() {
        return mysqli_error($GLOBALS['__MYSQLI_CONNECTION']);
    }
    
    function mysql_real_escape_string($string) {
        return mysqli_real_escape_string($GLOBALS['__MYSQLI_CONNECTION'], $string);
    }
    
    function mysql_get_server_info() {
        return mysqli_get_server_info($GLOBALS['__MYSQLI_CONNECTION']);
    }
    
    function mysql_set_charset($charset) {
        return mysqli_set_charset($GLOBALS['__MYSQLI_CONNECTION'], $charset);
    }
    
    function mysql_unbuffered_query($query) {
        return mysqli_query($GLOBALS['__MYSQLI_CONNECTION'], $query, MYSQLI_USE_RESULT);
    }
}

// Додаємо глобальне визначення константи, яка використовується в sxd_error_handler
if(!defined('SXD_DEBUG')) define('SXD_DEBUG', false);
...
class Sypex_Dumper {
	// Сучасний конструктор для PHP 7+
	function __construct() {
		define('C_DEFAULT', 1);
		define('C_RESULT', 2);
		define('C_ERROR', 3);
		define('C_WARNING', 4);
		// SXD_DEBUG визначено глобально
	...
	// Підтримка застарілого конструктора для PHP 5 і нижче
...
function sxd_error_handler($errno, $errmsg, $filename, $linenum, $vars = null){
    global $SXD;
    
    // Ігноруємо деякі типи помилок
    if($errno == 8192) return; // Deprecated
    if(strpos($errmsg, 'timezone settings')) return; // Помилки з часовими поясами
    
    // Перевіряємо стан об'єкту SXD
    $is_sxd_valid = isset($SXD) && is_object($SXD);
    $is_trying = $is_sxd_valid && isset($SXD->try) && $SXD->try;
    
    // Якщо ми в режимі "try", виходимо
    if($is_trying) return;
    
    // Типи помилок
    $errortype = array(
        1 => 'Error', 2 => 'Warning', 4 => 'Parsing Error', 8 => 'Notice', 
        16 => 'Core Error', 32 => 'Core Warning', 64 => 'Compile Error', 
        128 => 'Compile Warning', 256 => 'MySQL Error', 512 => 'Warning', 
        1024 => 'Notice', 2048 => 'Strict', 8192 => 'Deprecated', 16384 => 'Deprecated'
    );
    
    // Форматуємо повідомлення про помилку
    $error_type = isset($errortype[$errno]) ? $errortype[$errno] : 'Unknown Error';
    $str = "{$error_type}: {$errmsg} ({$filename}:{$linenum})";
    $esc_str = addcslashes($str, "\\\0\n\r\t\'");
    
    // Логування помилок, якщо увімкнено режим відлагодження
    if(defined('SXD_DEBUG') && SXD_DEBUG) {
        error_log("[index.php]\n{$str}\n", 3, "backup/error.log");
    }
    
    // Перевіряємо логи і файлові хендлери
    $has_log_handlers = $is_sxd_valid && isset($SXD->fh_log) && isset($SXD->fh_rtl);
    $has_active_logs = $has_log_handlers && ($SXD->fh_log || $SXD->fh_rtl);
    
    // Обробка незначних помилок (Notice)
    if($errno == 8 || $errno == 1024) {
        if(!$has_active_logs) {
            echo isset($_POST['ajax']) ? "alert('{$esc_str}');" : $str;
        } elseif($is_sxd_valid && isset($SXD->fh_log) && $SXD->fh_log) {
            fwrite($SXD->fh_log, date('Y.m.d H:i:s') . "\t3\t{$str}\n");
        }
    }
    // Обробка важливих помилок
    elseif($errno < 1024) {
        if($is_sxd_valid) {
            $SXD->error = true;
        }
        ...
        if(!$has_active_logs) {
            ...
            // Оновлення RTL файлу
            if($is_sxd_valid && isset($SXD->rtl) && isset($SXD->fh_rtl) && $SXD->fh_rtl) {
                $SXD->rtl[1] = time();
                $SXD->rtl[9] = 5;
                fseek($SXD->fh_rtl, 0);
            ...
            // Запис в лог
            if($is_sxd_valid && isset($SXD->fh_log) && $SXD->fh_log) {
                fwrite($SXD->fh_log, date('Y.m.d H:i:s') . "\t4\t{$str}\n");
            }
            
            // Очищення RTL
            if($is_sxd_valid && isset($SXD->rtl)) {
                unset($SXD->rtl);
            }
        }
        
        // Зупинка виконання скрипта
        die;
    }
}
...
function sxd_shutdown(){
	...
	// Перевіряємо, чи існує об'єкт $SXD і необхідні властивості
	if(!isset($SXD) || !is_object($SXD)) return;
	
	// Перевіряємо наявність обробника файлу та масиву rtl
	if(isset($SXD->fh_rtl) && is_resource($SXD->fh_rtl) && 
	   isset($SXD->rtl) && !empty($SXD->rtl) && 
	   (!isset($SXD->error) || empty($SXD->error))) {
	    
		// Оновлюємо час
		$SXD->rtl[1] = time();
		
		// Перевіряємо наявність файлу зупинки
		if(isset($SXD->JOB) && isset($SXD->JOB['file_stp']) && 
		   file_exists(dirname(__FILE__) . '/' . $SXD->JOB['file_stp'])){
